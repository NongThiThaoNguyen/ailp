...
export default function LessonPage() {
  ...
  useEffect(()=>{
    timer.current=setInterval(()=>setTimeSpent(p=>p+1),1000)
  ...
  return (
    <div className="flex gap-6 h-[calc(100vh-140px)] overflow-hidden">
      ...
      <aside className="w-72 bg-bg-card border border-bdr rounded-xl flex flex-col h-full overflow-y-auto shrink-0">
        <div className="p-5 border-b border-bdr">
          <Link href="/courses/1" className="flex items-center gap-2 text-sm text-tx-muted hover:text-brand-light mb-3 font-medium transition-colors">← Quay lại</Link>
      ...
      {/* Lesson details pane */}
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-bg-card border border-bdr rounded-xl">
        <header className="px-6 py-4 border-b border-bdr flex items-center justify-between shrink-0">
          <div className="font-bold text-sm">3. Grammar for IELTS — Relative Clauses</div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-tx-muted">Tuần 4/12 · 60%</span>
            <button onClick={markDone} className="btn-primary text-xs px-3.5 py-2">Đánh dấu hoàn thành ✓</button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {/* Tabs */}
          <div className="flex gap-0 bg-bg-card2 rounded-xl p-1 w-fit mb-5 border border-bdr">
            {TABS.map(t=><button key={t} onClick={()=>setTab(t)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab===t?'bg-brand text-white':'text-tx-muted hover:text-tx-base'}`}>{t}</button>)}
          </div>

          {tab==='Nội dung bài học' && <div className="space-y-4">
            <div className="card bg-bg-card2"><h4 className="font-bold mb-2">Mệnh đề quan hệ (Relative Clauses)</h4><p className="text-sm text-tx-sec leading-relaxed">Mệnh đề quan hệ dùng để bổ sung thông tin cho danh từ đứng phía trước, sử dụng các đại từ quan hệ: <strong>who, whom, which, that, whose, where, when</strong>.</p></div>
            <div className="card bg-bg-card2"><h4 className="font-bold mb-2">Ví dụ thực tế</h4><div className="space-y-2 text-sm text-tx-sec"><p>• The man <em className="text-brand-light font-semibold">who is standing</em> over there is my uncle.</p><p>• The book <em className="text-brand-light font-semibold">that I read</em> is interesting.</p><p>• This is the house <em className="text-brand-light font-semibold">where I was born</em>.</p></div></div>
            <div className="card bg-bg-card2"><h4 className="font-bold mb-2">Phân loại đại từ quan hệ</h4><div className="grid grid-cols-2 gap-2 text-sm">{[['who / whom','Người (chủ ngữ/tân ngữ)'],['which','Vật, sự việc'],['that','Người hoặc vật'],['whose','Sở hữu'],['where','Nơi chốn'],['when','Thời gian']].map(([w,d])=><div key={w} className="flex gap-2 p-2 bg-bg-card3 rounded-lg"><code className="text-brand-light font-mono font-bold shrink-0">{w}</code><span className="text-tx-muted">{d}</span></div>)}</div></div>
          </div>}
          
          {tab==='Tài liệu' && <div className="space-y-3">{[{name:'Bài tập Relative Clauses.pdf',size:'245 KB',icon:'📄'},{name:'Vocabulary List.docx',size:'128 KB',icon:'📝'},{name:'Audio: Listening Practice.mp3',size:'12 MB',icon:'🎵'}].map(f=><a key={f.name} href="#" className="flex items-center gap-3 p-4 card bg-bg-card2 hover:border-brand transition-colors cursor-pointer"><span className="text-2xl">{f.icon}</span><div className="flex-1"><div className="text-sm font-semibold">{f.name}</div><div className="text-xs text-tx-muted">{f.size}</div></div><span className="text-tx-muted hover:text-brand-light">⬇</span></a>)}</div>}
          
          {tab==='Ghi chú' && <div className="card bg-bg-card2"><h4 className="font-bold mb-3">Ghi chú của bạn</h4><textarea className="input min-h-[200px] resize-y bg-bg-card3" placeholder="Ghi chú bài học tại đây..." value={note} onChange={e=>setNote(e.target.value)}/><button className="btn-primary mt-3">💾 Lưu ghi chú (AI sẽ tóm tắt)</button></div>}
