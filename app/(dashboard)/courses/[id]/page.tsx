        ...
        <div className="p-7 animate-fadeUp">
          ...
          <div className="grid grid-cols-[1fr_320px] gap-6">
            <div>
              ...
              <div className="space-y-3">{MODULES.map(m=>(
                <div key={m.id} className="card">
                  ...
                  <div className="space-y-2">{m.lessons.map(l=>(
            ...
            <div className="space-y-4">
              <div className="card"><h3 className="font-bold mb-3">Yêu cầu đầu vào</h3><ul className="space-y-2 text-sm text-tx-muted">{['Tiếng Anh cơ bản (A2+)','Có mục tiêu rõ ràng','Kiên trì và chăm chỉ'].map(r=><li key={r} className="flex items-center gap-2"><span className="text-ok">✓</span>{r}</li>)}</ul></div>
              <div className="card"><h3 className="font-bold mb-3">Bạn sẽ học được</h3><ul className="space-y-2 text-sm text-tx-muted">{['Đạt IELTS 6.5 Overall','Chiến lược làm bài hiệu quả','Từ vựng học thuật nâng cao','Kỹ năng giao tiếp tự nhiên'].map(r=><li key={r} className="flex items-center gap-2"><span className="text-brand-light">→</span>{r}</li>)}</ul></div>
            </div>
          </div>
        </div>
      </div>
