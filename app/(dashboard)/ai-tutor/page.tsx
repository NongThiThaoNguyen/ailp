...
export default function AITutorPage() {
  ...
  const send = async (text:string) => {
    if (!text.trim()) return
    setMsgs(p=>[...p,{id:Date.now(),role:'user',text,time:hhmm()}]); setInput(''); setTyping(true)
    try {
      ...
      setTyping(false); setMsgs(p=>[...p,{id:Date.now()+1,role:'ai',text:aiText,time:hhmm()}])
    } catch { setTyping(false); setMsgs(p=>[...p,{id:Date.now()+1,role:'ai',text:FALLBACK[0],time:hhmm()}]) }
  }
  return (
    <div className="flex min-h-screen bg-bg-dark">
      <Sidebar/>
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="flex items-center justify-between px-7 py-4 bg-bg-card border-b border-bdr shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div><div className="font-bold">AI Tutor</div><div className="text-xs text-ok font-medium">● Đang hoạt động</div></div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-7 py-5 space-y-4">
          ...
          {typing && <div className="flex gap-3 max-w-[78%]"><div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand to-second flex items-center justify-center text-xl shrink-0">🤖</div><div className="bubble-ai flex items-center gap-1.5 py-4"><span className="dot"/><span className="dot"/><span className="dot"/></div></div>}
          <div ref={bot}/>
        </div>
        <div className="px-7 py-4 border-t border-bdr bg-bg-card shrink-0">
          <div className="flex gap-2 mb-3 flex-wrap">{QUICK.map(q=><button key={q} onClick={()=>send(q)} className="text-xs px-3 py-1.5 rounded-full border border-bdr bg-bg-card2 text-tx-muted hover:border-brand hover:text-brand-light transition-all">{q}</button>)}</div>
          <div className="flex gap-3 items-center bg-bg-card2 border border-bdr rounded-2xl px-4 py-2.5 focus-within:border-brand transition-colors">
